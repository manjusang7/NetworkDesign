...
def timer():                                                    # another thread for timer
    ...
    event2.wait()                                               # wait for green light
    couting_time = time.time()
    while event2.is_set():
        if (time.time() - couting_time) > timeout:
            print("Timeout. Resend Packet No." + str(t))
